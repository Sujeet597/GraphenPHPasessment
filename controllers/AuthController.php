...
class Auth
{
    ...
    public function login($email, $password){
        ...
        // Sanitize and prepare user inputs
        $email = strtolower(trim(DB::sanitize($email)));
        $password = DB::sanitize($password);
    
        // Execute the query
        $loginQuery = DB::select('users', '*', "email = '$email'");
        ...
        if ($loginQuery !== false) {
            ...
            if ($userData) {
                ...
                $PasswordFromDB = $userData[0]['password'];
    
                // Verify the provided password against the hashed password from the database
                if (password_verify($password, $PasswordFromDB)) {
                    // Passwords match, proceed with authentication
                    $loginID = md5(sha1($email) . sha1($password) . sha1(time()));
                    setcookie("auth", $loginID, time() + (86400 * 365), "/");
    
                    $ip = getDevice()['ip'];
                    $os = getDevice()['os'];
                    $browser = getDevice()['browser'];
